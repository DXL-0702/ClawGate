...
| v1.0 | ✅ Phase 1-3 完成 | 团队部署（Phase 2）· 健康面板 + 自动离线检测（Phase 3） |
| **v1.0 Delivery Readiness** | 🔧 当前 | 缺口修复（Web UI 入 Docker / Issue 6 双模式认证 / 团队 Compose）→ 技术债 → 功能完善 → 文档 → 端到端 QA |
| **v1.0 GA** | 🎯 目标 | 所有 Delivery Readiness 阶段验收通过即交付 |
| v1.x Phase 4 | 🔜 GA 后 | SDK · Watchtower 自动更新 · 跨实例日志聚合 · Remote SSH · 插件机制 |
